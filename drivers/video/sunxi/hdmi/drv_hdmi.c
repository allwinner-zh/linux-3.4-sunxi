...
void hdmi_delay_ms(unsigned long t)
{
...
static int hdmi_parse_io_config(void)
{
	...
	for(i=0; i<HDMI_IO_NUM; i++) {
		...
		ret = OSAL_Script_FetchParser_Data("hdmi_para", io_name, (int *)gpio_info, sizeof(disp_gpio_set_t)/sizeof(int));
		if(ret == 0)
		  hdmi_io_used[i]= 1;
		else
			hdmi_io_used[i] = 0;
