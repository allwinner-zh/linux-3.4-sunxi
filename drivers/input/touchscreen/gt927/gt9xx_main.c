...
char *firmware_name = "GT927_1030_D2A9.BIN";
...
static void gtp_set_int_value(int status)
{
...
{	    
	 ...
	 dprintk(DEBUG_INIT,"==========------TS Interrupt-----============\n");  
...
//    return ret;
    ...
    while(retry++ < 5)
    {
        ...
        if (ret > 0)
        {
            printk("Wakeup sleep send config success.");
            return ret;
        }
    }
...
//    u8 cfg_info_group1[] = CTP_CFG_GROUP1;
    ...
    u8 *send_cfg_buf[3] = {gt9xx_fw_grp[fw_index].data, cfg_info_group2, cfg_info_group3};
    u8 cfg_info_len[3] = {gt9xx_fw_grp[fw_index].size, 
...
#endif
    dprintk(DEBUG_INIT,"ts->abs_x_max = %d,ts->abs_y_max = %d\n",ts->abs_x_max,ts->abs_y_max);
