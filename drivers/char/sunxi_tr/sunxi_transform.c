...
static int sunxi_tr_clk_enable(char *id)
{
	...
	if(NULL == hclk || IS_ERR(hclk)) {
		pr_warn("Fail to get handle for clock %s.\n", id);
		return -1;
	}

	ret = clk_prepare_enable(hclk);
	if(0 != ret)
		pr_warn("Fail enable clock %s.\n", id);
	clk_put(hclk);

	return ret;
}

static int sunxi_tr_clk_disable(char *id)
{
	struct clk* hclk = NULL;
	int ret = 0;
...
int sunxi_tr_request(void)
{
	...
	if(1 == count) {
		sunxi_tr_clk_enable(gsunxi_dev->clk);
		de_tr_init();
...
static int __devinit tr_probe(struct platform_device *pdev)
{
...
#else
	sunxi_dev->base = kzalloc(4*1024*1024, GFP_KERNEL);
	if (!sunxi_dev->base) {
	...
	dev_init = true;
	/* init hw */
	de_tr_set_base((unsigned int)sunxi_dev->base);
