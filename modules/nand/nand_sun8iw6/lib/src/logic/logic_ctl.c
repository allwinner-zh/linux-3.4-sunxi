...
__s32 LML_VirtualPageRead(struct __PhysicOpPara_t *pVirtualPage)
{
    ...
    spare_buf = pVirtualPage->SDataPtr;
    ...
    if(spare_buf)
    	MEMCPY(pVirtualPage->SDataPtr, tmpPhyPage.SDataPtr, 8);
...
__s32 LML_VirtualPageWrite( struct __PhysicOpPara_t *pVirtualPage)
{
    ...
    if(spare_buf)
    {
    	...
    	tmpPhyPage.SDataPtr = LML_SPARE_BUF;
		MEMCPY(LML_SPARE_BUF, pVirtualPage->SDataPtr,8);
    }
    else
    {
    	tmpPhyPage.SDataPtr = LML_SPARE_BUF;
        MEMSET(LML_SPARE_BUF, 0xff, 8);
    }
...
static __s32 _CloseWritePage(void)
{
    ...
    if(LogicalCtl.OpMode == 'w')
    {
        ...
        PHY_SynchBank_write(tmpPage.BankNum, SYNC_CHIP_MODE);
    }
...
__s32 LML_PageWrite(__u32 nPage, __u64 nBitmap, void* pBuf)
{
	...
	if(tmpLogicPage.BlockNum==tmpLogicalBlk)
	{
		if(tmpLogicalBlk!= 0xffff)
			LML_MergeLogBlk_Quit();
	}
    ...
    if(LogicalCtl.LogPageNum == 0)
    {
        ...
        tmpSpare[1].PageStatus = tmpSpare[0].PageStatus + 1;
    }
    else
    {
...
__TRY_WRITE_PHYSIC_PAGE:
	...
	//if ((!SUPPORT_WRITE_CHECK_SYNC)||(LogicalCtl.LogBlockType == LSB_TYPE))
...
__s32 LML_Init(void)
{
    ...
    LogicalCtl.DiskCap = SECTOR_CNT_OF_SINGLE_PAGE * PAGE_CNT_OF_PHY_BLK * BLOCK_CNT_OF_DIE * \
            DIE_CNT_OF_CHIP * NandDriverInfo.NandStorageInfo->ChipCnt*CHANNEL_CNT/ BLOCK_CNT_OF_ZONE * DATA_BLK_CNT_OF_ZONE;
...
__s32 LML_Exit(void)
{
	if(tmpFreeBlklock)
	{
		PRINT("LML_Exit fail because tmpFreeBlklock is %d\n", tmpFreeBlklock);
		while(1);
	}

	if((tmpLogicalBlk!=0xffff)||((tmpMergePageNum!=0xffff)&&(tmpMergePageNum!=0xefef))||(tmpMergeFreeBlk.PhyBlkNum!=0xffff))
    	{
		PRINT("LML_Exit fail, tmp Merge parameters error!\n");
...
__u32 NAND_GetLogicPageCnt(void)
{
	return (PAGE_CNT_OF_SUPER_BLK);
}
...
__s32 NAND_SetPartInfo(void *part_table)
{
	...
	{
		LOGICCTL_ERR("Nand Part magic error!\n");
		return -1;
	}
	if(nandpart->part_cnt > NAND_MAX_PART_CNT)
	{
		LOGICCTL_ERR("Nand Part Cnt is invalid!\n");
		return -1;
	}
	...
	for(i=0; i<nandpart->part_cnt;i++)
	{
		...
		if(nandpart->start_sec[i]%(PAGE_CNT_OF_LOGIC_BLK*SECTOR_CNT_OF_LOGIC_PAGE))
			LOGICCTL_ERR("nand part %d start sec is not align\n");
		if(nandpart->sec_cnt[i]%(PAGE_CNT_OF_LOGIC_BLK*SECTOR_CNT_OF_LOGIC_PAGE))
			LOGICCTL_ERR("nand part %d sec count is not align\n");
